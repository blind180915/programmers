...
public class Greedy42860 {
	...
	public static int solution(String name) {
        ...
        for(int i = 0; i < name.length(); ++i) {
        	if(name.charAt(i) != 'A') {
        		answer += (int)(name.charAt(i) - 'A') < (int)('Z' - name.charAt(i) + 1) ? 
        				  (int)(name.charAt(i) - 'A') : (int)('Z' - name.charAt(i) + 1);
        		pos.add(i);
        	}
        }
        ...
        int rightIndex = 0;
        int left = 0;
        int leftIndex = name.length() - 1;
        
        for(int i = 0; i < pos.size(); ++i) {
        	right += pos.get(i) - rightIndex;
        	rightIndex = pos.get(i);
        }
        ...
        for(int i = pos.size() - 1; i >= 0; --i) {
        	...
        	left += leftIndex - pos.get(i);
        	leftIndex = pos.get(i);
        }
        
        return answer + Math.min(right, left + 1);
    }
